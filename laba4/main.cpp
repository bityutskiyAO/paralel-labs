...
int main() {
    ...
    vector<vector<double>> solved1(odu.pikar(f, alpha, t0, tm, step, false));
    vector<vector<double>> transposed_solved1(odu.transpose(solved1));
    vector<vector<double>> solved2(odu.pikar(f, alpha, t0, tm, step, true));
    vector<vector<double>> transposed_solved2(odu.transpose(solved2));

    vector<function<double(const double)>> u{ u1, u2, u3, u4, u5 };
    vector<double> calculated;
    for (ptrdiff_t i_func(0); i_func < 5; i_func += 1) {
        calculated = odu.calculate(u[i_func], t0, tm, step);
        ...
        cout << "Consequential\nu" << i_func + 1 << ": " << odu.maximumMetric(calculated, transposed_solved1[i_func]) << endl;
        // Максимальное значение модуля разницы между точными значениями
        // и значениями численного решения при параллельном подходе
        cout << "Parallel\nu" << i_func + 1 << ": " << odu.maximumMetric(calculated, transposed_solved2[i_func]) << endl << endl;
    }
    cout << endl;
    ...
    for (ptrdiff_t i(0); i < 5; i += 1) {
        start = omp_get_wtime();
        odu.pikar(f, alpha, t0, tm, step, false);
        time_sequental += omp_get_wtime() - start;

        start = omp_get_wtime();
        odu.pikar(f, alpha, t0, tm, step, true);
