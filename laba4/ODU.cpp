...
double metric(const vector<double> &x, const vector<double> &y, const bool is_parallel) {
    double res(0.0);
    size_t n(x.size());
...
//vector<double> ODU::pikar(int n, double t0, double tm, double capacity, vector<double> &alpha,
//                          vector<std::function<double(double, const vector<double> &)>> &f) {
//    double step = (tm - t0) / capacity;
//    vector<double> t;
//    double curr_t(t0);
//    while (curr_t < tm) {
//        t.push_back(curr_t);
...
//
//    vector<vector<double>> integrals(total_n_points - 1, vector<double>(n_u));
...
//
//    vector<vector<double>> sum(1, vector<double>(n_u));
//
//    while (condition) {
//
...
////        {
//            ptrdiff_t m(0);
//            double diff(metric(u[m], prev_u[m], is_parallel));
//            while (((diff < epsilon) && (m + 1) < total_n_points)) {
//                m += 1;
//                diff = metric(u[m], prev_u[m], is_parallel);
//            }
...
vector<vector<double>> ODU::pikar(const std::vector<std::function<double(const double, const std::vector<double> &)>> &f,
           const std::vector<double> &alpha, const double t0, const double tm, const double step,
           const bool is_parallel) {
    // Количество потоков
    const int32_t n_threads(is_parallel ? 1 : 1);
...
double ODU::maximumMetric(const std::vector<double> &x, const std::vector<double> &y) {
...
std::vector<std::vector<double>> ODU::transpose(const std::vector<std::vector<double>> &matrix) {
...
std::vector<double>
ODU::calculate(const std::function<double(const double)> &u, const double t0, const double tm, const double step) {
    vector<double> ans;
    double curr_t(t0);
    while (curr_t < tm) {
