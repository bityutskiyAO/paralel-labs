...
void testVectorPush(ThreadSaveVector * testVector) {
    ...
    if(testVector->threadCount == 5) {
        clock_t end = clock();
        testVector->printVector();
        double generalTime = (double) (end - start) / CLOCKS_PER_SEC;
        printf("General pushing time: %f seconds\n", generalTime);
        double lockedTime(0);
        for (int i = 0; i < testVector->pushingTime.size(); ++i) {
            printf("%d thread pushing time : %f seconds\n", i, testVector->pushingTime[i]);
            lockedTime += testVector->pushingTime[i];
        }
        printf("Сoefficient of effective use: %f seconds\n", lockedTime / generalTime);
    }
